...
import { createManifestScript, generateManifest, renderManifestHTML } from '../../src/component/ssr.js';
...
test('SSR: renderManifestHTML escapes closing script tags', t => {
    ...
    t.true(html.includes('<\\/script>'), 'Should escape </script> to prevent breaking the container tag');
...
test('SSR: createManifestScript generates correct script and escapes tags', t => {
    ...
    t.true(defaultScript.textContent.includes('window.__HYDRATION_DATA__ ='), 'Should use default variable name');

    // 2. Test custom variable name
    const customVar = 'MY_APP_DATA';
    const customScript = createManifestScript(manifest, customVar);
    t.true(customScript.textContent.includes(`window.${customVar} =`), 'Should use custom variable name');

    // 3. Test </script> escaping (The regex logic)
    // The string should be escaped to <\/script>
    t.false(customScript.textContent.includes('</script>'), 'Should not contain raw closing script tags');
    t.true(customScript.textContent.includes('<\\/script>'), 'Should escape closing script tags for safety');
