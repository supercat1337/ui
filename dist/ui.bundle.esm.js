...
var Config = class {
    ...
    _mergeConfigObj(config, element) {
        const jsonConfig = isElement(element)
            ? manipulator_default.getDataAttribute(element, "config")
            : {};
        return {
...
var enableDismissTrigger = (component, method = "hide") => {
    ...
    event_handler_default.on(
        ...
        function (event) {
            ...
            const target =
                selector_engine_default.getElementFromSelector(this) ||
                this.closest(`.${name}`);
...
var FocusTrap = class extends config_default {
    ...
    activate() {
        ...
        event_handler_default.on(document, EVENT_FOCUSIN, (event) =>
            this._handleFocusin(event)
        );
        event_handler_default.on(document, EVENT_KEYDOWN_TAB, (event) =>
...
var ScrollBarHelper = class {
    ...
    _saveInitialAttribute(element, styleProperty) {
        ...
        if (actualValue) {
            manipulator_default.setDataAttribute(
                element,
                styleProperty,
                actualValue
            );
    ...
    _resetElementAttributes(selector, styleProperty) {
        const manipulationCallBack = (element) => {
            const value = manipulator_default.getDataAttribute(
                element,
                styleProperty
            );
            if (value === null) {
    ...
    _applyManipulationCallback(selector, callBack) {
        ...
        for (const sel of selector_engine_default.find(
            selector,
            this._element
        )) {
            callBack(sel);
...
var Modal = class _Modal extends base_component_default {
    constructor(element, config) {
        ...
        this._dialog = selector_engine_default.findOne(
            SELECTOR_DIALOG,
            this._element
        );
    ...
    _showElement(relatedTarget) {
        ...
        const modalBody = selector_engine_default.findOne(
            SELECTOR_MODAL_BODY,
            this._dialog
        );
        if (modalBody) {
...
var EventEmitter = class {
    ...
    on(event, listener) {
        ...
        if (
            !/^(#has-listeners|#no-listeners)$/.test(event) &&
            this.events[event].length == 1
        ) {
...
var Component = class {
     ...
     * @param {import("dom-scope/dist/dom-scope.esm.js").RefsAnnotation} [refsAnnotation2] - An array of strings representing the names of the refs.
     * The function is called with the component instance as the this value.
     */
    setLayout(layout, refsAnnotation2) {
        let template;
        ...
        if (refsAnnotation2) {
            this.refsAnnotation = refsAnnotation2;
        }
    }
    /**
...
var PagedData = class {
    ...
    constructor(param0) {
        ...
        let total =
            typeof param0.total == "number"
                ? param0.total
                : parseInt(param0.total);
        let page_size =
    ...
    notify(response) {
        if (response && response.id && typeof response.id == "string") {
            if (
                ...
                setTimeout(
                    () => this.#eventEmitter.emit(response.id, response),
                    0
                );
            }
        }
...
function extractRPCResponse(object = {}, rpcOptions = {}) {
    rpcOptions.notify = rpcOptions.hasOwnProperty("notify")
        ? rpcOptions.notify
        : true;
    ...
    if (responseType === "error") {
        response = new RPCErrorResponse(
            object.error,
            object.id || rpcOptions.id
        );
